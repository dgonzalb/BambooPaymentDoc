...
All methods used in Purchase API require an authentication header configured using the following parameters.
...
| `Order` | `string` | Sí | Order number generated by the merchant. |
| `Amount` | `number` | Sí | Amount of the purchase. This value must be greater than zero.<br>If you must include decimals in the amount, concatenate the decimal places without de decimal point. Example `12,25` > `1225`. |
| `Currency` | `string` | Sí | Currency of the purchase, according to ISO-4217. Find the posibles valores the [Currencies](#) table. |
| `Installments` | `integer` | No | This parameter refers to the number of payments that a credit card purchase is divided into. |
| `Capture` | `boolean` | No | Defines whether the purchase should be performed in one or two steps.<sup>2</sup><br><ul style="margin-bottom: initial;"><li>If `false`, only the authorization is processed, and the purchase is pre-authorized until the final confirmation through the commit and rollback calls.</li><li>If `true`, the transaction is authorized and captured (committed).</li></ul><br>All [payment methods and countries](/es/docs/payment-methods.html) may not support the pre-authorization feature. |
| `TargetCountryISO` | `string` | Sí | This parameter indicates the country where the payment will be processed.<br>Send the country using `ISO-3166-1` format. |
...
| `Customer` → `BillingAddress` | `object` | No | Este parámetro es la dirección de facturación del cliente associated with their card. |
...
| `Customer` → `DocumentTypeId` | `string` | No | Tipo de documento del cliente. Find the posibles valores in the [Document types](#) table. |
...
| `AdditionalData` | `string` | No | You can add additional information to the transaction (Por ejemplo a `Key:Value` data list).<br>We return this information processing the purchase each time you consult the purchase. |
...
* <sup>2</sup> All the payment methods may not support the pre-authorization feature. Review the [Countries and payment methods](/es/docs/payment-methods.html) section to check availability.
* <sup>3</sup> This parameter is mandatory when creating _CrossBorder purchases_.
* <sup>4</sup> This object is not required if you create the purchase using [_CommerceToken_](#).
...
##### Request example
You must include the new amount in the request to confirm a purchase with a lower amount than the original. Por ejemplo:
...
| `Response` → `MetadataOut` | `object` | Additional fields returned by each payment method or acquirer to perform the following steps.<br>Por ejemplo, this object can have the URL to where you need to redirect the customer or the QR image that has to be scanned by your customer. |
