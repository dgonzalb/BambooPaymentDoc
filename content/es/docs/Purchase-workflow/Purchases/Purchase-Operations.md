...
| `Currency` | `string` | Yes | Currency of the purchase, according to ISO-4217. Find the posibles valores the [Currencies](#) table. |
...
| `Customer` → `BillingAddress` | `object` | No | This parameter is the customer's billing address associated with their card. |
...
| `Customer` → `DocumentTypeId` | `string` | No | Customer document type. Find the posibles valores in the [Document types](#) table. |
...
* <sup>2</sup> All the payment methods may not support the pre-authorization feature. Review the [Countries and payment methods](/docs/payment-methods.html) section to check availability.
