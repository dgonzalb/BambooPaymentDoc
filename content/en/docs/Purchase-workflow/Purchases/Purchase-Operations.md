...
| `Customer` → `DocumentTypeId` | `string` | No | Customer document type. Find the possible values in the [Document types](#) table. |
...
| `AdditionalData` | `string` | No | You can add additional information to the transaction (For example a `Key:Value` data list).<br>We return this information processing the purchase each time you consult the purchase. |
